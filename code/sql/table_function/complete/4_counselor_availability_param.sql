...
CREATE OR REPLACE TYPE t_counselor_available_table 
AS TABLE OF t_counselor_available_row;
/


CREATE OR REPLACE Function counselor_availability(
  p_day_start int default 9,
  p_day_end int default 17
)
  ...
  BEGIN
     ...
     LOOP
       ...
       LOOP
         ...
         v_break_start := v_start_date + p_day_start * (1/24);
         FOR appt_record
         IN appt_cursor
         LOOP
         ...
         v_break_duration := ((v_start_date + p_day_end * (1/24)) - v_break_start) * 24 * 60;
         if v_break_duration >= 30 then
           v_ret.extend;
           v_ret(v_ret.count):=t_counselor_available_row(v_counselor, v_break_start, v_break_duration);
         end if;
...
select * from table(counselor_availability(8,18))
